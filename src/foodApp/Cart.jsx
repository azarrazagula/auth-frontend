...
const Cart = ({ cartItems, onUpdateQuantity, onRemoveItem, onClose, onClearCart }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface/80 backdrop-blur-sm p-2 sm:p-4">
      <div className="w-full max-w-lg bg-surface max-h-[95vh] sm:max-h-[90vh] shadow-2xl overflow-y-auto animate-in zoom-in-95 duration-300 rounded-2xl sm:rounded-[32px] border border-outline-variant/10">

        {/* Header */}
        <div className="sticky top-0 bg-surface/90 backdrop-blur-md p-4 sm:p-6 border-b border-outline-variant/20 flex items-center justify-between z-10">
          <h2 className="font-headline text-xl sm:text-2xl font-black text-on-surface">Your Cart</h2>
          <button
            onClick={onClose}
            className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full hover:bg-surface-container-high text-on-surface transition-colors"
          >
        ...
        <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
              ...
              cartItems.map((item) => (
                <div key={item.id} className="flex gap-4 p-4 rounded-2xl bg-surface-container-lowest border border-outline-variant/10">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h4 className="font-headline font-bold text-xs sm:text-sm text-on-surface line-clamp-1">{item.name}</h4>
                      <p className="font-body text-[10px] sm:text-xs text-on-surface-variant">${item.price.toFixed(2)}</p>
                    </div>
          ...
          {/* Pricing Summary */}
          {cartItems.length > 0 && (
            <div className="space-y-3 p-4 sm:p-6 rounded-2xl bg-surface-container border border-outline-variant/10">
              <div className="flex justify-between font-body text-xs sm:text-sm text-on-surface-variant">
                <span>Subtotal</span>
              ...
              <div className="pt-3 border-t border-outline-variant/20 flex justify-between items-center">
                ...
                <span className="font-headline text-lg sm:text-xl font-black text-primary">${total.toFixed(2)}</span>
              </div>
            </div>
          )}
